-- 🎯 SCRIPT CORRIGIDO PARA SUPORTE A MÍDIA NO WHATSAPP
...
DROP POLICY IF EXISTS "Allow upload for whatsapp media" ON storage.objects;
DROP POLICY IF EXISTS "Allow delete for whatsapp media" ON storage.objects;
...
CREATE POLICY "Allow upload for whatsapp media" 
ON storage.objects 
FOR INSERT 
WITH CHECK (bucket_id = 'whatsapp-media');

-- Policy para delete (permite qualquer usuário - ajuste conforme necessário)
CREATE POLICY "Allow delete for whatsapp media" 
ON storage.objects 
FOR DELETE 
USING (bucket_id = 'whatsapp-media');
